...
test("typing in the explore search returns grouped results", async ({ page }) => {
  ...
  await expect(page.getByTestId("search-result-trail-buckeye-trail")).toBeVisible({ timeout: 20000 });
});
...
test("tapping a trail result navigates to that trail's detail page", async ({ page }) => {
  ...
  await expect(page.getByTestId("search-result-trail-towpath-trail")).toBeVisible({ timeout: 20000 });
...
test("tapping a system result navigates to that system's detail page", async ({ page }) => {
  ...
  await expect(page.getByTestId("search-result-system-cuyahoga-valley-national-park")).toBeVisible({ timeout: 20000 });
...
test("tapping a feature result navigates to that feature's detail page", async ({ page }) => {
  ...
  await expect(page.getByTestId(`search-result-feature-${FIXTURE_IDS.f1}`)).toBeVisible({ timeout: 20000 });
  await page.getByTestId(`search-result-feature-${FIXTURE_IDS.f1}`).click();
  await expect(page).toHaveURL(new RegExp(`/feature/${FIXTURE_IDS.f1}$`));
...
test("search shows an empty state when no results match", async ({ page }) => {
  ...
  await expect(page.getByTestId("search-empty")).toBeVisible({ timeout: 20000 });
});
