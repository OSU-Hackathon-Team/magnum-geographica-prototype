...
// Trace heatmap — cached tile-level density for the explore map overlay.
// Each row stores the count of active traces that intersect a tile at a
// given zoom level. Zoom 14 is the base level; lower zooms aggregate up.
// The table is regenerated by the admin heatmap job and updated
// incrementally when traces are created or change status.
export const traceHeatmap = pgTable(
  "trace_heatmap",
  {
  ...
  (t) => ({
    pk: primaryKey({ columns: [t.zoom, t.tileX, t.tileY] }),
  }),
);
...
export type TraceHeatmap = typeof traceHeatmap.$inferSelect;
export type NewTraceHeatmap = typeof traceHeatmap.$inferInsert;
