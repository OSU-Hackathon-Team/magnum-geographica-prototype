...
export const superSystems = pgTable("super_systems", {
...
export const systemSuperSystems = pgTable("system_super_systems", {
  systemId: uuid("system_id").notNull().references(() => systems.id, { onDelete: "cascade" }),
  superSystemId: uuid("super_system_id").notNull().references(() => superSystems.id, { onDelete: "cascade" }),
}, (t) => ({
...
export const trailSystems = pgTable("trail_systems", {
...
export const trailSubSystems = pgTable("trail_sub_systems", {
  trailId: uuid("trail_id").notNull().references(() => trails.id, { onDelete: "cascade" }),
  subSystemId: uuid("sub_system_id").notNull().references(() => subSystems.id, { onDelete: "cascade" }),
}, (t) => ({
  pk: primaryKey({ columns: [t.trailId, t.subSystemId] }),
}));

export const trailSegments = pgTable("trail_segments", {
  id: uuid("id").primaryKey().defaultRandom(),
  trailId: uuid("trail_id").notNull().references(() => trails.id, { onDelete: "cascade" }),
  name: text("name"),
  ...
  hazards: text("hazards").array().notNull().default(sql`'{}'::text[]`),
