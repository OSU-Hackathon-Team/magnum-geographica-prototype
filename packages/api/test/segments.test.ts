...
const buildApp = () =>
  new Hono()
    .route("/api/segments", segmentDetailRoute)
    .route("/api/trails", trailSegmentsRoute);
...
describe("POST /api/trails/:id/segments", () => {
  test("returns 404 when trail not found", async () => {
    const res = await buildApp().request("/api/trails/00000000-0000-0000-0000-000000000001/segments", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        geometry: {
...
describe("POST /api/trails/:id/segments/reorder", () => {
  test("rejects when ordered_ids is missing", async () => {
    const res = await buildApp().request("/api/trails/00000000-0000-0000-0000-000000000001/segments/reorder", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({}),
    });
...
describe("POST /api/trails/:id/segments/split", () => {
  test("rejects when segment_id is missing", async () => {
    const res = await buildApp().request("/api/trails/00000000-0000-0000-0000-000000000001/segments/split", {
      method: "POST",
      headers: { "content-type": "application/json" },
...
describe("POST /api/trails/:id/segments/merge", () => {
  test("rejects when one of the segment ids is missing", async () => {
    const res = await buildApp().request("/api/trails/00000000-0000-0000-0000-000000000001/segments/merge", {
      method: "POST",
      headers: { "content-type": "application/json" },
